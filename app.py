...
def index():
    ...
    content = playstore.copy()
    content['Genres'] = content['Genres'].astype('category')
    content = playstore.groupby(['Genres']).agg({'Price': 'mean'}).sort_values('Price', ascending=False).head(10).reset_index()
    content = content.set_index(['Genres'])

    genres = list(content.index) 
    price = list(content['Price'])

    fig = plt.figure(figsize=(20,7))
    plt.barh(genres, price, color='orange')
    plt.gca().invert_yaxis()
    plt.title('Top Selling Genres in Playstore', size=25)
